...
<head>
    ...
    <title><fmt:message key="title.update_manager_login"/></title>
</head>
<body>
...
<h1><fmt:message key="title.update_manager_login"/></h1>
<form action="${pageContext.request.contextPath}/controller?command=updating_manager_login" method="post">
    <c:if test="${requestScope.registrationError != null}">
        <div class="error"><fmt:message key="error.registration_user"/></div>
    </c:if>
    <c:if test="${requestScope.updatingmanagerLoginError != null}">
